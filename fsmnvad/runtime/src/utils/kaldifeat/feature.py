...
def func_dither(waveform, dither_value):
    if dither_value == 0.0:
        return waveform
    waveform += (
        np.random.normal(size=waveform.shape).astype(waveform.dtype) * dither_value
    )
    return waveform
...
def extract_window(
    ...
    frames = sliding_window(
        waveform, window_size=window_size, window_shift=window_shift
    )
...
def apply_cmvn_sliding_internal(
    ...
    if center:
        if num_frames <= window:
            ...
            if norm_vars:
                ...
                std3 = feat3.mean(axis=0, keepdims=True).repeat(
                    (window - 1) // 2, axis=0
                )
