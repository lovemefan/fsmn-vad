...
class E2EVadModel:
    ...
    def pop_data_to_output_buf(self, start_frm: int, frm_cnt: int, first_frm_is_start_point: bool,
    ...
    def on_voice_end(self, end_frame: int, fake_result: bool, is_last_frame: bool) -> None:
        ...
        for batch_num in range(0, feats.shape[0]):  # only support batch_size = 1 now
            ...
            if len(self.output_data_buf) > 0:
                for i in range(self.output_data_buf_offset, len(self.output_data_buf)):
                    if not self.output_data_buf[i].contain_seg_start_point or \
                            not self.output_data_buf[i].contain_seg_end_point:
                        continue
    ...
    def infer_online(self, feats: np.ndarray, waveform: np.ndarray, in_cache: Dict[str, np.ndarray] = dict(),
    ...
    def detect_one_frame(self, cur_frm_state: FrameState, cur_frm_idx: int, is_final_frame: bool) -> None:
        ...
        if cur_frm_state == FrameState.kFrameStateSpeech:
            ...
            tmp_cur_frm_state = FrameState.kFrameStateSil
        state_change = self.windows_detector.detect_one_frame(tmp_cur_frm_state, cur_frm_idx)
        ...
        if AudioChangeState.kChangeStateSil2Speech == state_change:
            ...
            start_frame = 0
            if self.vad_state_machine == VadStateMachine.kVadInStateStartPointNotDetected:
                start_frame = max(self.data_buf_start_frame, cur_frm_idx - self.latency_frm_num_at_start_point())
            ...
            if self.vad_state_machine == VadStateMachine.kVadInStateStartPointNotDetected:
                ...
                if ((self.vad_opts.detect_mode == VadDetectMode.kVadSingleUtteranceDetectMode.value) and (
                    ...
                    if cur_frm_idx >= self.latency_frm_num_at_start_point():
                        self.on_silence_detected(cur_frm_idx - self.latency_frm_num_at_start_point())
            elif self.vad_state_machine == VadStateMachine.kVadInStateInSpeechSegment:
